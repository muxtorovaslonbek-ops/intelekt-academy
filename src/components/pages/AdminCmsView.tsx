    ...
    <div id="admin-cms-view" className="max-w-7xl mx-auto space-y-6 pb-20">
                  ...
                  <div className="p-4 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex flex-col shrink-0 -mt-1">
                        <button
                          onClick={() => moveCourseUp(course.id)}
                          disabled={courseIndex === 0}
                          title="Kursni yuqoriga surish"
                        ...
                        <button
                          onClick={() => moveCourseDown(course.id)}
                          disabled={courseIndex === courses.length - 1}
                          title="Kursni pastga surish"
                          className="p-0.5 rounded text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/60 disabled:opacity-25 disabled:cursor-not-allowed transition-all cursor-pointer"
                        >
      ...
      {managingCourseLessons && (
        <LessonManagerModal
          ...
          onMoveLesson={(courseId, lessonId, direction) => {
            if (direction === 'up') {
              moveLessonUp(courseId, lessonId);
            } else {
              moveLessonDown(courseId, lessonId);
            }
          }}
        />
      )}
